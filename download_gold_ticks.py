...
# --- Settings ---
SYMBOL = "XAUUSD"
PRICE_DIVISOR = 1000.0  # Gold divisor is 1000 (3 decimal places)
MAX_WORKERS = 12        # Number of concurrent threads for downloading active hours
OUTPUT_FILE = "xauusd_ticks_1y.csv"
    ...
    for attempt in range(retries):
        try:
            ...
            if e.code == 404:
                return None
            if e.code == 503:
                # Rate limited: Cool down for a few seconds
...
def get_active_hours_for_date(date_obj):
    ...
    Forex/Gold market is open Sunday 22:00 UTC to Friday 22:00 UTC.
    """
...
def run_download(start_date_str, end_date_str, dry_run=False):
    ...
    print("╔══════════════════════════════════════════════════════╗")
    print("║   DUCASCOPY GOLD (XAUUSD) TICK DOWNLOADER            ║")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Dukascopy Gold Tick Downloader")
    parser.add_argument("--dry-run", action="store_true", help="Run a test download for 1 day only")
    args = parser.parse_args()
    
    # We want a full year from 1 year ago until today
