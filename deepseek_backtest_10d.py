...
def run_deepseek_backtest_10d():
    print("⏳ Loading dataset and setting up DeepSeek 10-day backtest...")
    df = pd.read_csv("processed_data/aligned_dataset.csv")
    df['timestamp'] = pd.to_datetime(df['timestamp'])
    df = df.sort_values('timestamp').reset_index(drop=True)
    
    # Slice for the last 10 days
    ...
    # Find the starting index that has at least 10 historical rows within the 10d range
    start_bar_indices = df_10d[df_10d['timestamp'] >= start_time].index.tolist()
    if not start_bar_indices:
        print("❌ No bars found in the last 10 days.")
        return
        
    start_idx = start_bar_indices[0]
    # We will test at every 3rd bar to avoid overlapping trades and speed up API calls (approx 80-100 total calls)
    test_indices = list(range(start_idx, len(df_10d) - 5, 3))
    ...
    print(f"📈 Testing DeepSeek decisions at {len(test_indices)} points over the last 10 days...")
    
    results = []
    # Run in parallel using a ThreadPoolExecutor with 12 workers for faster throughput
    with ThreadPoolExecutor(max_workers=12) as executor:
        futures = {
            executor.submit(fetch_decision, api_key, df_10d, idx, len(df_10d)): idx 
    ...
    while i < len(results):
        ...
        # Volatility Breakout Filter: Block trades on massive breakout bars (>30 bps)
        if bar_ret_abs > 0.0030:
            guardrail_passed = False
            reason_blocked = f"Extreme breakout volatility: bar return is {bar_ret_abs*10000:.1f} bps (> 30 bps)"
        elif decision == 'SHORT' and ofi_current < 0.02:
            guardrail_passed = False
        ...
        if decision in ['BUY', 'SHORT'] and confidence >= 0.70:
            ...
            exit_idx = min(entry_idx + lead_horizon, len(df_10d) - 1)
            
            entry_price = res['close']
            exit_price = df_10d.loc[exit_idx, 'close']
            
            spread_entry = res['spread']
            spread_exit = df_10d.loc[exit_idx, 'spread']
    ...
    print("\n=======================================================")
    print("       DEEPSEEK AGENT 10-DAY BACKTEST SUMMARY          ")
    print("=======================================================")
