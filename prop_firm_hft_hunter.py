...
def fetch_decision(api_key, df_slice, current_bar_idx):
    """
    Sends enriched HFT-sensing context to DeepSeek and requests trade setup.
    """
    ...
    data_table = context_bars[[
        ...
        'gold_ofi_accel_5', 'quote_imbalance_velocity_5', 
        'price_impact_coef_5', 'vwap_dev_20',
        'vol_ratio_5_20', 'gold_dxy_corr_20'
    ]].to_string(index=False)
    
    prompt = f"""You are a Lead Quantitative Research Scientist and Head of HFT Strategy at a prop trading firm.
...
You must dynamically analyze the cross-asset indicators and HFT order book footprints to detect the dominant market regime (MEAN_REVERSION vs. TREND_FOLLOWING) and execute the appropriate trade setup.

Here is the enriched HFT-hunter dataset of the last 10 Gold (XAUUSD) Dollar Bars, showing rolling correlations, book imbalances, and order-book toxicity:

{data_table}

Follow these scientific trading guidelines to decode HFT intentions:
1. HFT Regime Detection:
   ...
   - Identify TREND_FOLLOWING: Volatility ratio ('vol_ratio_5_20') is expanding (> 1.2), price is far from VWAP, and HFT momentum algorithms are aggressively sweep-buying/selling as indicated by large, persistent spikes in 'gold_ofi_accel_5' and 'quote_imbalance_velocity_5'.
2. Execution Rules:
   - If MEAN_REVERSION: Fade extremes and target reversion to VWAP ('vwap_dev_20' approaching 0). BUY oversold pullbacks (z_score < -0.8) or SHORT overbought rallies (z_score > 0.8).
   - If TREND_FOLLOWING: Join the momentum. BUY breakouts (price > h_ema_20 with positive OFI acceleration) or SHORT breakdowns (price < h_ema_20 with negative OFI acceleration).
3. HFT Intention Warnings:
   - If price impact coefficient ('price_impact_coef_5') is extremely large while OFI is near zero, it indicates a liquidity vacuum. HFTs are pulling order books. Fading this is highly toxic (banned).
...
{{
  ...
  "analysis": "Brief analysis of HFT footprints (OFI acceleration, VWAP distance, and price impact elasticity)",
  "decision": "BUY", "SHORT", or "HOLD",
  "confidence": <float between 0.0 and 1.0>,
  "stop_loss_atr": <float, between 1.0 and 3.0>,
  "take_profit_atr": <float, between 1.5 and 6.0>,
  "reasoning": "Detailed justification of HFT intentions, regime detection, trade decision, and SL/TP selection"
}}
...
def run_hunter_backtest(period_name="LAST_MONTH"):
    ...
    print(f"📈 Running HFT-Hunter Strategy at {len(test_indices)} points...")
    ...
    print("\n=======================================================")
    print(f"      HFT-HUNTER BACKTEST SUMMARY: {period_name}")
    print("=======================================================")
...
if __name__ == '__main__':
    run_hunter_backtest("LAST_MONTH")
    run_hunter_backtest("PREVIOUS_MONTH")
