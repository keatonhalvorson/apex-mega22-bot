...
def fetch_decision(api_key, df_slice, current_bar_idx):
    """
    Sends macro lead-lag and microstructure data to DeepSeek to predict trend direction and make trade decisions.
    """
    ...
    data_table = context_bars[[
        ...
        'gold_ofi_norm_1', 'gold_ofi_norm_5', 'gold_ofi_accel_5', 
...
Here is the enriched market microstructure dataset of the last 10 Gold (XAUUSD) Dollar Bars, showing rolling correlations, book imbalances, cross-asset returns, and order-flow:

{data_table}

Analyze the data with scientific rigor, focusing on predicting the trend direction using lead-lag indicators and HFT order book footprints:
1. Trend Prediction & Lead-Lag Analysis:
   - Gold-DXY Lead: DXY price movements lead Gold. If DXY return is negative ('dxy_ret_3' < 0.0) and Gold OFI is positive, it anticipates a Gold bullish trend.
   - Gold-Bond Lead: Bond prices lead Gold. If UST Bond return is positive ('bond_ret_3' > 0.0) and Gold OFI is positive, it anticipates Gold strength.
   - Order Flow Accumulation: 'gold_ofi_norm_5' is the 5-bar accumulated OFI. A value > 0.35 indicates HFTs are quietly buying the book (anticipating a bullish trend), and < -0.35 indicates HFTs are selling (anticipating a bearish trend).
2. Regime Identification:
   - Identify MEAN_REVERSION: Volatility ratio ('vol_ratio_5_20') is low (< 1.1), price is close to VWAP ('vwap_dev_20' between -0.8 and 0.8), and order flow acceleration ('gold_ofi_accel_5') is low/oscillating.
   - Identify TREND_FOLLOWING: Volatility ratio ('vol_ratio_5_20') is expanding (> 1.05) OR 'gold_ofi_norm_5' is extremely high/low (absolute value > 0.40, indicating order flow breakout), and price is far from VWAP.
3. CRITICAL SAFETY RULES (MUST BE STRICTLY ENFORCED, NO EXCEPTIONS ALLOWED):
   - Macro Trend Alignment (Allows Pullback Dip Buying):
     * Bullish Trend (close > h_ema_20): You can enter a BUY trade. You are also allowed to BUY a deep pullback (even if close is slightly below h_ema_20) if 'z_score' < -0.8 (cheap entry in a long-term bullish trend). No SHORT trades allowed.
     * Bearish Trend (close < h_ema_20): You can enter a SHORT trade. You are also allowed to SHORT a high rally (even if close is slightly above h_ema_20) if 'z_score' > 0.8 (cheap short entry in a long-term bearish trend). No BUY trades allowed.
   ...
   - Volatility Squeeze Block (TREND_FOLLOWING): You MUST NOT enter a trend-following trade unless 'vol_ratio_5_20' > 1.05 OR absolute 'gold_ofi_norm_5' > 0.40.
...
def run_predictor_backtest(period_name="LAST_MONTH"):
    ...
    print(f"📈 Running Trend-Predictor Strategy (NO Python Filters) at {len(test_indices)} points...")
    ...
    print("\n=======================================================")
    print(f"      TREND-PREDICTOR BACKTEST SUMMARY: {period_name}")
    print("=======================================================")
...
if __name__ == '__main__':
    run_predictor_backtest("LAST_MONTH")
    run_predictor_backtest("PREVIOUS_MONTH")
