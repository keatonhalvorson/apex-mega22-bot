...
def fetch_decision(api_key, df_slice, current_bar_idx):
    """
    Sends HFT and cross-asset indicators to DeepSeek, prompting it to behave as an elite HFT Quant Scientist.
    """
...
Unlike standard retail systems, there are NO code-level filters blocking your trades. You are fully responsible for risk management, regime detection, and capital preservation.

Here is the enriched market microstructure dataset of the last 10 Gold (XAUUSD) Dollar Bars, showing rolling correlations, book imbalances, and order-book toxicity:

{data_table}

Analyze the data with scientific rigor, focusing on HFT intentions and cross-asset correlations:
...
2. HFT Intention Decoupling:
   - Check 'price_impact_coef_5': If the price impact coefficient is extremely large while order flow acceleration is small, HFT market makers are pulling their limit orders (creating a liquidity vacuum). Do NOT attempt to fade this move; it is a breakout.
   - Check 'quote_imbalance_velocity_5': A sudden spike indicates spoofing or massive book reconfiguration by HFTs.
3. CRITICAL SAFETY RULES (MUST BE STRICTLY ENFORCED, NO EXCEPTIONS ALLOWED):
   - Macro Trend Alignment (Absolute for ALL regimes including MEAN_REVERSION):
   ...
   - Overextension Block (TREND_FOLLOWING): You MUST NOT buy if 'rsi_14' > 62 or 'z_score' > 1.5. You MUST NOT short if 'rsi_14' < 38 or 'z_score' < -1.5.
   - Volatility Squeeze Block (TREND_FOLLOWING): You MUST NOT enter a trend-following trade unless 'vol_ratio_5_20' > 1.15. No exceptions.
...
def run_ai_scientist_backtest(period_name="LAST_MONTH"):
    ...
    print(f"📈 Running AI-Scientist Strategy (NO Python Code Filters) at {len(test_indices)} points...")
    ...
    print("\n=======================================================")
    print(f"      AI-SCIENTIST BACKTEST SUMMARY: {period_name}")
    print("=======================================================")
...
if __name__ == '__main__':
    run_ai_scientist_backtest("LAST_MONTH")
    run_ai_scientist_backtest("PREVIOUS_MONTH")
