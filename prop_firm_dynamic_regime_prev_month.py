...
def fetch_decision(api_key, df_slice, current_bar_idx):
    """
    Sends enriched 10-bar context to DeepSeek and requests dynamic regime detection and trade setup.
    """
...
def run_dynamic_backtest_prev_month():
    ...
    # Filter for previous month (from days 60 to 30 ago)
    last_time = df['timestamp'].max()
    end_time = last_time - pd.Timedelta(days=30)
    start_time = end_time - pd.Timedelta(days=30)
    df_prev = df[(df['timestamp'] >= start_time - pd.Timedelta(hours=10)) & (df['timestamp'] <= end_time)].copy().reset_index(drop=True)
    
    start_bar_indices = df_prev[df_prev['timestamp'] >= start_time].index.tolist()
    if not start_bar_indices:
        print("❌ No bars found in the previous month period.")
        return
        
    start_idx = start_bar_indices[0]
    test_indices = list(range(start_idx, len(df_prev) - 15, 4))
    ...
    print(f"📈 Testing dynamic regime strategy over the PREVIOUS month ({start_time.date()} to {end_time.date()}) at {len(test_indices)} points...")
    
    results = []
    with ThreadPoolExecutor(max_workers=12) as executor:
        futures = {
            executor.submit(fetch_decision, api_key, df_prev, idx): idx 
    ...
    while i < len(results):
        ...
        volume = df_prev.loc[entry_idx, 'volume']
        close = df_prev.loc[entry_idx, 'close']
        open_price = df_prev.loc[entry_idx, 'open']
        h_ema = df_prev.loc[entry_idx, 'h_ema_20']
        bar_ret_abs = abs(np.log(close / open_price))
        
        guardrail_passed = True
        reason_blocked = ""
        
        # Determine if the trade is trend-following or counter-trend
        ...
        # Guardrail logic:
        # 1. Block all counter-trend trades (strictly forbidden to ensure funded account safety)
        if not is_trend_aligned:
            guardrail_passed = False
            reason_blocked = f"Counter-trend trade blocked: {decision} is forbidden on the wrong side of hourly EMA (price {close:.2f} vs EMA {h_ema:.2f})"
        # 2. Block mean-reversion trades on extreme breakout bars (> 30 bps)
        elif regime == 'MEAN_REVERSION' and bar_ret_abs > 0.0030:
            guardrail_passed = False
            reason_blocked = f"Volatility breakout on Mean-Reversion: return is {bar_ret_abs*10000:.1f} bps (> 30 bps)"
            
        if decision in ['BUY', 'SHORT'] and confidence >= 0.75:
            ...
            for f_idx in range(entry_idx + 1, min(entry_idx + 16, len(df_prev))):
                f_row = df_prev.iloc[f_idx]
                f_high = f_row['high']
                f_low = f_row['low']
            ...
            if outcome is None:
                outcome = 'TIME_EXIT'
                exit_idx = min(entry_idx + 15, len(df_prev) - 1)
                exit_price = df_prev.loc[exit_idx, 'close']
                
            cost = (res['spread'] / entry_price + df_prev.loc[exit_idx, 'spread'] / exit_price) / 2.0 + 2 * commission
            if decision == 'BUY':
    ...
    print("\n=======================================================")
    print("   PREVIOUS MONTH DYNAMIC CHALLENGE BACKTEST SUMMARY   ")
    print("=======================================================")
...
if __name__ == '__main__':
    run_dynamic_backtest_prev_month()
