...
def test_real_l2_snapshots_data_integrity():
    ...
    candidates_l2 = [
        os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "l2_live_data", "btcusdt_l2_snapshots_2026-07-31.jsonl"),
        os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "..", "l2_live_data", "btcusdt_l2_snapshots_2026-07-31.jsonl"),
        os.path.join("/home/atheer/Desktop/ApexPredator/download data", "l2_live_data", "btcusdt_l2_snapshots_2026-07-31.jsonl")
    ]
    l2_file = next((p for p in candidates_l2 if os.path.exists(p)), None)
    if l2_file is None:
        pytest.skip("L2 snapshots file not found in environment")
    ...
    candidates_metrics = [
        os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "processed_data", "l2_analyzed_metrics.csv"),
        os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "..", "processed_data", "l2_analyzed_metrics.csv"),
        os.path.join("/home/atheer/Desktop/ApexPredator/download data", "processed_data", "l2_analyzed_metrics.csv")
    ]
    metrics_file = next((p for p in candidates_metrics if os.path.exists(p)), None)
    if metrics_file is None:
        pytest.skip("Analyzed metrics file not found in environment")
