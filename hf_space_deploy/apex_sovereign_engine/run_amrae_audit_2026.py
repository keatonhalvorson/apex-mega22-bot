"""
====================================================================================================
         APEX SOVEREIGN REGIME ENGINE (AMRAE v1.0) - 2026 OUT-OF-SAMPLE AUDIT
         Official 2026 Audit (Jan 1, 2026 through August 28, 2026)
====================================================================================================
Performance Metrics:
- Starting Capital:   $1,000.00
- Ending Capital:     $1,036.93
- Net 2026 Profit:    +$36.93 (+3.69% RoE MTD)
- Profitable Months:  6 / 8 months (75.0% Monthly Win Rate)
- 2-Year Consistency: 15 / 20 months (75.0% Win Rate across 2025-2026)
====================================================================================================
"""
...
DATA_DIR = Path("/home/atheer/Desktop/Apex_Autonomous_Agent/apex_v2/data/full_year_2026")
OUTPUT_DIR = Path(__file__).parent / "results_2026"
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

ALPHA_UNIVERSE = [
    "ATOMUSDT", "NEARUSDT", "RENDERUSDT"
]
...
def run_amrae_month(ym: str, start_cap: float, base_risk_budget: float = 15.0, monthly_loss_cap: float = -12.0, fee=0.0004, slip=0.0002):
    ...
    for s in symbols:
        ...
        for t in range(1, n_bars):
            if raw_setup_vals[t - 1]:
                if is_green_vals[t] and (c_vals[t] >= c_vals[t - 1] * 0.9995) and (rs_vals[t] >= -1.0):
                    sig[t] = 1
    ...
    for i in range(1440, n_bars - 1):
        ...
        for sym, pos in list(active_positions.items()):
            ...
            is_stalled = (held >= 25) and (cur_pnl < 0.0005) and (worst_pnl < -0.007)
...
def main():
    ...
    print("=======================================================================================")
    print("🚀 RUNNING AMRAE v1.0 AUDIT ON 2026 DATA (Jan 1, 2026 to Aug 28, 2026)...")
    print("=======================================================================================")
    ...
    # Save CSVs
    df_all.to_csv(OUTPUT_DIR / "amrae_2026_all_trades.csv", index=False)
    df_ms.to_csv(OUTPUT_DIR / "amrae_2026_monthly_summary.csv", index=False)
    ...
    total_months = len(df_ms)
    
    print("\n=======================================================================================")
    print("🏆 AMRAE v1.0 - 2026 OUT-OF-SAMPLE AUDIT REPORT (Jan 1 to Aug 28, 2026):")
    print("=======================================================================================")
    print(df_ms.to_string(index=False))
    print("\n=======================================================================================")
    print(f"💰 Starting Capital:              $1,000.00")
    print(f"💎 Ending Capital:                ${capital:0.2f}")
    print(f"📈 Total 2026 Net Profit:         ${tot_pnl:+0.2f}")
    print(f"🚀 Total 2026 Return on Equity:   +{tot_ret:0.2f}%")
    print(f"🎯 2026 Win Rate:                 {tot_wr}% ({tot_wins}/{tot_trades} trades)")
    print(f"📊 2026 Profit Factor:             {tot_pf}")
    print(f"📅 Profitable Months:             {profitable_months} / {total_months} months ({profitable_months/total_months*100:0.1f}%)")
